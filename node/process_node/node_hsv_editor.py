...
def image_process(image, h_add, s_add, v_add):
    ...
    processed_hsv_image = cv2.merge([h, s, v])
    bgr_image = cv2.cvtColor(processed_hsv_image, cv2.COLOR_HSV2BGR)
...
class Node(DpgNodeABC):
    ...
    def update(
        ...
        for connection_info in connection_list:
            ...
            if connection_type == self.TYPE_INT:
                ...
                # スライダーの範囲に合わせて値をクリップ
                if 'Input02' in destination_tag:
                    min_val, max_val = -90, 90
                else:
                    min_val, max_val = -255, 255
                
                input_value = max([min_val, input_value])
                input_value = min([max_val, input_value])
                
                dpg_set_value(destination_tag, input_value)
            if connection_type == self.TYPE_IMAGE:
                # 画像取得元のノード名(ID付き)を取得
                connection_info_src = connection_info[0]
                connection_info_src = connection_info_src.split(':')[0:2]
        ...
        # スライダーの値取得
        h_add = int(dpg_get_value(input_value02_tag))
        s_add = int(dpg_get_value(input_value03_tag))
        v_add = int(dpg_get_value(input_value04_tag))
        ...
        if frame is not None:
            frame = image_process(frame, h_add, s_add, v_add)
    ...
    def get_setting_dict(self, node_id):
        ...
        h_val = dpg_get_value(input_value02_tag)
        s_val = dpg_get_value(input_value03_tag)
        v_val = dpg_get_value(input_value04_tag)
        ...
        setting_dict[input_value02_tag] = h_val
        setting_dict[input_value03_tag] = s_val
        setting_dict[input_value04_tag] = v_val
    ...
    def set_setting_dict(self, node_id, setting_dict):
        ...
        h_val = int(setting_dict[input_value02_tag])
        s_val = int(setting_dict[input_value03_tag])
        v_val = int(setting_dict[input_value04_tag])

        dpg_set_value(input_value02_tag, h_val)
        dpg_set_value(input_value03_tag, s_val)
        dpg_set_value(input_value04_tag, v_val)
